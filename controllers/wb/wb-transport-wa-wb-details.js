// Services
const wbTransportWaWbDetailsService = require("../../services/wb/wb-transport-wa-wb-details");

// Validations
const wbTransportWaWbDetailsValidation = require("../../validations/wb/wb-transport-wa-wb-details");
...
exports.create = async (request, response) => {
  ...
  const results = await wbTransportWaWbDetailsService.create(bodyPalod);
...
exports.selectByRequisitionId = async (request, response) => {
    ...
    // call service
    const results = await wbTransportWaWbDetailsService.selectByRequisitionId(id);
...
exports.selectWithFabricManufacturedByRequisitionId = async (request, response) => {
    ...
    // call service
    const results = await wbTransportWaWbDetailsService.selectWithFabricManufacturedByRequisitionId(id);
    response.status(200).json(results);
  };
  ...
  exports.update = async (request, response) => {
    ...
    // Validation
    if (!wbTransportWaWbDetailsValidation.isValidUpdate(bodyPalod)) {
      // logging
      return response.status(400).json(constants.invalidDataResponse);
    }
    bodyPalod.id = id;
    const updateResults = await wbTransportWaWbDetailsService.update(bodyPalod);
    // response
