// Validation
const dyeingServicesPricesValidation = require("../../validations/general/dyeing-services-prices");

// Services
const dyeingServicesPricesService = require("../../services/general/dyeing-services-prices");
...
exports.select = async (request, response) => {
  ...
  // call service
  const results = await dyeingServicesPricesService.select();
...
exports.selectDeleted = async (request, response) => {
  ...
  // call service
  const results = await dyeingServicesPricesService.selectDeleted();
  response.status(200).json(results);
};
...
exports.create = async (request, response) => {
  ...
  // Validation
  if (!dyeingServicesPricesValidation.isValid(bodyPalod)) {
    // logging
    return response.status(400).json(constants.invalidDataResponse);
  }
  //   send data to service
  const results = await dyeingServicesPricesService.create(bodyPalod);
...
exports.update = async (request, response) => {
  ...
  // Validation
  if (!dyeingServicesPricesValidation.isValidUpdate(bodyPalod)) {
    // logging
    return response.status(400).json(constants.invalidDataResponse);
  }
  bodyPalod.id = id
  const updateResults = await dyeingServicesPricesService.update(bodyPalod);
  // response
...
exports.delete = async (request, response) => {
  ...
  const results = await dyeingServicesPricesService.dalete(bodyPalod);
...
exports.restore = async (request, response) => {
  ...
  // call services
    const results = await dyeingServicesPricesService.restore(bodyPalod);
