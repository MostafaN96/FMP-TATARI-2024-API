...
// Validation
const fabricYarnsValidation = require("../../validations/general/fabric-yarns");
...
exports.create = async (request, response) => {
  ...
  // Validation
  if (!fabricYarnsValidation.isValid(bodyPalod)) {
    // logging
  ...
  const results = await fabricYarnsService.create(bodyPalod);
...
exports.selectByFabricId = async (request, response) => {
  // logging
  const { fabricId } = request.params;

  // call service
  const results = await fabricYarnsService.selectByFabricId(fabricId);
...
exports.update = async (request, response) => {
  ...
  bodyPalod.id = id
  const updateResults = await fabricYarnsService.update(bodyPalod);
  // response
...
exports.delete = async (request, response) => {
  ...
  const results = await fabricYarnsService.dalete(bodyPalod);
