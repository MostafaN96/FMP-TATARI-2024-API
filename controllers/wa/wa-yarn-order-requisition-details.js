...
const waYarnOrderRequisitionDetailsService = require("../../services/wa/wa-yarn-order-requisition-details");

// Validations
const waYarnOrderRequisitionDetailsValidation = require("../../validations/wa/wa-yarn-order-requisition-details");
...
exports.create = async (request, response) => {
  ...
  // Validation
  if (!waYarnOrderRequisitionDetailsValidation.isValid(bodyPalod)) {
      // logging
...
exports.selectByRequisitionIdOpenedOrder = async (request, response) => {
    ...
    // call service
    const results = await waYarnOrderRequisitionDetailsService.selectByRequisitionIdOpenedOrder(id);
...
exports.selectByRequisitionIdClosedOrder = async (request, response) => {
    ...
    // call service
    const results = await waYarnOrderRequisitionDetailsService.selectByRequisitionIdClosedOrder(id);
    response.status(200).json(results);
  };
  ...
  exports.closeOrder = async (request, response) => {
    ...
    // call service
    const results = await waYarnOrderRequisitionDetailsService.closeOrder(id);
    response.status(200).json(results);
  };
  ...
  exports.update = async (request, response) => {
    ...
    // Validation
    if (!waYarnOrderRequisitionDetailsValidation.isValidUpdate(bodyPalod)) {
      // logging
      return response.status(400).json(constants.invalidDataResponse);
    }
    bodyPalod.id = id;
    const updateResults = await waYarnOrderRequisitionDetailsService.update(bodyPalod);
    // response
...
exports.selectByYarnySeller = async (request, response) => {
  ...
  // call service
  const results = await waYarnOrderRequisitionDetailsService.selectByYarnySeller(fabricId, sellerId);
  response.status(200).json(results);
};
