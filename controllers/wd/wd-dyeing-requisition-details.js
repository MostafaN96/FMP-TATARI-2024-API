...
exports.create = async (request, response) => {
  ...
  // Validation
  if (!wdDyeingRequisitionDetailsValidation.isValid(bodyPalod)) {
      // logging
      return response.status(400).json(constants.invalidDataResponse);
  }
  //   send data to service
  const results = await wdDyeingRequisitionDetailsService.create(bodyPalod);
...
exports.selectByRequisitionId = async (request, response) => {
    ...
    // call service
    const results = await wdDyeingRequisitionDetailsService.selectByRequisitionId(id);
  ...
  exports.update = async (request, response) => {
    ...
    // Validation
    if (!wdDyeingRequisitionDetailsValidation.isValidUpdate(bodyPalod)) {
      // logging
      return response.status(400).json(constants.invalidDataResponse);
    }
    bodyPalod.id = id;
    const updateResults = await wdDyeingRequisitionDetailsService.update(bodyPalod);
    // response
  ...
  exports.updateDyeingCostPrice = async (request, response) => {
    // logging
    const { id } = request.params;

    const updateResults = await wdDyeingRequisitionDetailsService.updateDyeingCostPrice(id);
    // response
    switch (updateResults) {
      case constants.itemNotFound:
        return response.status(200).json(constants.itemNotFound);
