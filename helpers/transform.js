...
exports.transform = () => {
    ...
    return (`${years}${month}${days.getDate()}${hours}${mins}${seconds}${mseconds}${parseInt(Math.random() * 100)}`);
...
exports.generateCertificateNumber = () => {
    days = new Date();
    years = new Date().getFullYear()
    month = addZero(new Date().getMonth(), 2)
    day = addZero(days.getDate(), 2)
    hours = addZero(days.getHours(), 2);
    mins = addZero(days.getMinutes(), 2);
    seconds = addZero(days.getSeconds(), 2);
    const y = removeYearPrefix(years)
    
    return (`${y}${month}${day}${hours}${mins}${seconds}`);
