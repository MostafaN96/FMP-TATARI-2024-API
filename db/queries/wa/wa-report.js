...
// Config
const { yarnTableName, bussinessmanTableName, wbManufacturingOutputTableName, wbManufacturingInputOutputTableName, wbManufacturingInputTableName } = require("../../../util/database-tables-name");
...
exports.yarnOrdersReport = async (whereCluse) => {
    ...
    await knex.select(columns).from(function () {
      ...
      knex.raw(
                    `CASE WHEN ${waYarnOrderRequisitionDetailsTableName}.current_quantity < ${0}
                                                THEN coalesce( ( 
                                                ( ${waYarnOrderRequisitionDetailsTableName}.current_quantity * -1 / 
                                                ( (${waYarnOrderRequisitionDetailsTableName}.current_quantity * -1) + ${waYarnOrderRequisitionDetailsTableName}.initial_quantity) 
                                                 )
                                                 ) * 100 )
                                                ELSE ${0}
...
exports.yarnOrdersForGeneralOrderReport = async (whereCluse, yarns_ids) => {    
    ...
    await knex.select(columns).from(function () {
            ...
            [
                ...
                //   END as executed_quantity`),
                  knex.raw(
                    ...
                    END as current_quantity`),
                            knex.raw(
                                `SUM(${wbManufacturingInputTableName}.quantity_with_waste) as net_current_quantity`
                                // `CASE WHEN ${waYarnOrderRequisitionDetailsTableName}.current_quantity < ${0}
                                // THEN coalesce( (${wbManufacturingInputTableName}.quantity_with_waste * -1) + ${waYarnOrderRequisitionDetailsTableName}.initial_quantity )
                                // ELSE coalesce( ${waYarnOrderRequisitionDetailsTableName}.initial_quantity - ${waYarnOrderRequisitionDetailsTableName}.current_quantity )
                                // END as net_current_quantity`
                            ),
                    knex.raw(
      ...
      knex.raw(
            ...
            .distinct(`${waYarnOrderRequisitionDetailsTableName}.id as wa_yarn_order_requisition_details_id`)
            .from(`${waYarnOrderRequisitionTableName}`)
            .innerJoin(`${waYarnOrderRequisitionDetailsTableName}`,
                `${waYarnOrderRequisitionDetailsTableName}.wa_yarn_order_requisition_id`,
                `${waYarnOrderRequisitionTableName}.id`)
            ...
            .innerJoin(`${yarnTableName}`,
                ...
                `${wbManufacturingInputOutputTableName}.wa_yarn_order_requisition_details_id`,
            ...
            .where(`${waYarnOrderRequisitionDetailsTableName}.initial_quantity`, ">", 0)
            .whereIn(`${yarnTableName}.id`, yarns_ids)
            .as('t1')              
    }).as('temp')
    .where(whereCluse)
        .sum("needed_quantity as needed_quantity")
...
exports.yarnOrdersTotalForGeneralOrderReport = async (whereCluse) => {    
...
//       knex.raw(`
//     COALESCE(
//       SUM(CASE WHEN ${waYarnOrderRequisitionDetailsTableName}.current_quantity < ${0}
...
//       END),
//       0
//     ) as over_current_quantity_ratio
//   `)
